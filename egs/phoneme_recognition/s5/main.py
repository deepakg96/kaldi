...
with open("main_log.txt", "w") as f:
    ...
    if phoneme_transcription == True:
        gz_location = "exp/chain_cleaned/tdnn_1d_sp/decode_test_tgsmall/lat.1.gz"
        words_txt_loc = "{0}/exp/chain_cleaned/tdnn_1d_sp/graph_tgsmall/phones.txt".format(ORIGINAL_DIRECTORY)
        command = "../../../src/latbin/lattice-to-phone-lattice exp/chain_cleaned/tdnn_1d_sp/final.mdl ark:'gunzip -c exp/chain_cleaned/tdnn_1d_sp/decode_test_tgsmall/lat.1.gz |' ark:- | ../../../src/latbin/lattice-best-path --acoustic-scale=0.1 ark:- 'ark,t:|utils/int2sym.pl -f 2- exp/chain_cleaned/tdnn_1d_sp/graph_tgsmall/phones.txt > phones_transcript.txt'"
        bash_out = s.run(command, stdout=f, text=True, shell=True)
